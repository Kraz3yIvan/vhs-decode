...
//   - modelPath is cached as a static string so thread_local session
//     creation doesn't need QCoreApplication (which is main-thread-only
//     on some Qt builds).
// =============================================================================
...
static std::string               g_modelPathUtf8;   // cached at env-init time
        ...
        if (!g_envReady.load(std::memory_order_relaxed)) {
            try {
                ...
                QString modelPathQ = QCoreApplication::applicationDirPath()
                                   + "/chroma_net.onnx";
                g_modelPathUtf8 = modelPathQ.toStdString();
    ...
    if (g_envReady.load(std::memory_order_acquire) && !tl_sessionReady) {
        try {
            ...
            tl_session = new Ort::Session(
                *g_ortEnv, g_modelPathUtf8.c_str(), session_options);
